...
OUT_DIR = os.getenv("OUT_DIR", "./mn_dataset_out_v5")
SEED = int(os.getenv("SEED", "42"))
random.seed(SEED)

TARGET_COUNTS = {
    "train": int(os.getenv("TARGET_TRAIN", "6000")),
    "dev": int(os.getenv("TARGET_DEV", "240")),
    "test": int(os.getenv("TARGET_TEST", "240")),
}
...
def contains_self_questioning(notes: str) -> bool:
    lt = (notes or "").lower()
    return any(q in lt for q in SELF_Q_TRIGGERS) or ("?" in notes and ":" in notes)

def enforce_self_questioning(actions: List[dict]) -> bool:
    for a in actions:
        if a.get("command") == "ui_display_notes":
            c = (a.get("parameters", {}) or {}).get("content","")
            if contains_self_questioning(c):
                return True
    return False

def validate_stage_progress_pair(stage: str, progress: Any) -> Tuple[str, int]:
    stage_defaults = {
        "not_in_lesson": 0,
        "intuition": 10,
        "main_lesson": 40,
    ...
    canonical = stage_defaults[stage]
    if stage == "practice" and p >= 70 and p < 100:
        p = max(70, min(90, p))
    else:
        p = canonical
    return stage, p

# NEW CHECK: hard validator for a single turn’s content
def validate_turn_example(ex: dict) -> Tuple[bool, str]:
    meta = ex.get("meta", {})
    domain, topic, grade = meta.get("domain",""), meta.get("topic",""), meta.get("grade","")
    # Grade-topic gate
    allowed = valid_topics_for(domain, grade)
    if topic not in allowed:
        return False, f"grade-topic-invalid: {domain}/{topic} for {grade}"
    # Self-questioning presence
    actions = ex.get("output", {}).get("actions", [])
    saw_selfq = False
    for a in actions:
        if a.get("command") == "ui_display_notes":
            if contains_self_questioning(a.get("parameters", {}).get("content","")):
                saw_selfq = True
                break
    if not saw_selfq:
...
def validate_actions_payload(payload: dict) -> Tuple[bool, dict, str]:
    ...
    for a in payload["actions"]:
        ...
        if cmd == "update_status":
            ...
            if isinstance(ups, dict):
                ...
                stg, prog = validate_stage_progress_pair(stg, prog)
                ups["lesson_stage"] = stg
                ups["current_lesson_progress"] = str(prog)
                params["updates"] = ups
    ...
    if not enforce_self_questioning(repaired):
        return False, {"actions": repaired}, "Missing self-questioning tone"
...
# NEW CHECK: remove soft fallback; hard-fail if no valid mapping in N tries
...
def build_single_seed() -> Dict[str, Any]:
    ...
    stage, prog = validate_stage_progress_pair(status.get("lesson_stage","intuition"), status.get("current_lesson_progress",10))
    status["lesson_stage"] = stage
    status["current_lesson_progress"] = prog
...
def build_messages(seed_turn: dict) -> List[Dict[str, Any]]:
    ...
    rubric = (
        ...
        "and then answer it succinctly before proceeding. Connect ideas progressively: one question leads to a key concept; "
        "ask another question that bridges to the next concept."
    )
    return [
        {"role":"user","parts":[
            SYSTEM_PROMPT,
...
async def call_model_and_clean(turn_id: str, seed_turn: dict) -> Tuple[Optional[dict], dict, str]:
    ...
    # NEW CHECK: validate single example quality
    ok_single, reason_single = validate_turn_example(cleaned)
    if not ok_single:
        return None, {"raw": raw_payload, "reason": reason_single}, f"Turn validation failed: {reason_single}"
...
async def producer_chain(split: str, num_chains: int, min_len=3, max_len=5):
    ...
    while created < num_chains:
        ...
        for step in range(length):
            ...
            # NEW CHECK: enforce topic consistency in-chain
            if cleaned["meta"]["domain"] != domain or cleaned["meta"]["topic"] != topic:
                with open(rej_path, "a", encoding="utf-8") as frj:
                    frj.write(json.dumps({"reason":"topic-drift", "raw": cleaned}, ensure_ascii=False)+"\n")
                break
            ...
            inp_title = cleaned["input"]["status_dictionary"].get("current_lesson_title","")
            if inp_title and inp_title != topic:
                with open(rej_path, "a", encoding="utf-8") as frj:
                    frj.write(json.dumps({"reason":"title-drift", "raw": cleaned}, ensure_ascii=False)+"\n")
                break

            # Also inspect update_status for title drift if present

            upd = extract_first_update_status(cleaned["output"]["actions"])
            if upd:
                ups = upd.get("parameters",{}).get("updates",{})
                new_title = ups.get("current_lesson_title")
                if new_title and new_title != topic:
                    with open(rej_path, "a", encoding="utf-8") as frj:
                        frj.write(json.dumps({"reason":"title-drift", "raw": cleaned}, ensure_ascii=False)+"\n")
                    break
                ...
                try:
                    ...
                    nxt = "intuition"
                stg_n, prog_n = validate_stage_progress_pair(nxt, status.get("current_lesson_progress",0))
                status["lesson_stage"] = stg_n
                status["current_lesson_progress"] = prog_n

            # NEW CHECK: monotonic stage/progress
            try:
                ps = stage_order.index(prev_stage)
                cs = stage_order.index(status["lesson_stage"])
            except ValueError:
                with open(rej_path, "a", encoding="utf-8") as frj:
                    frj.write(json.dumps({"reason":"bad-stage", "raw": cleaned}, ensure_ascii=False)+"\n")
                break
            if cs < ps or (cs == ps and status["current_lesson_progress"] < prev_prog):
                with open(rej_path, "a", encoding="utf-8") as frj:
                    frj.write(json.dumps({"reason":"stage/progress-regression", "raw": cleaned}, ensure_ascii=False)+"\n")
                break
            prev_stage, prev_prog = status["lesson_stage"], status["current_lesson_progress"]

            # Write only after checks pass
            with open(out_chain, "a", encoding="utf-8") as fcln:
                fcln.write(json.dumps(cleaned, ensure_ascii=False)+"\n")

            # Next user action based on stage
...
async def main():
    ...
    # Only run single producers if needed
    if train_single > 0:
